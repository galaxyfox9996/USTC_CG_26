    ...
    else if (has_preview_)
    {
        draw_list->AddLine(
            ImVec2(
                ...
                config.bias[1] + points_.back().y),
            ImVec2(
                config.bias[0] + preview_point_.x,
                config.bias[1] + preview_point_.y),
            color,
            config.line_thickness);
    }
}
...
void Polygon::set_closed(bool closed)
{
    is_closed_ = closed;
    has_preview_ = !closed;
}
...
std::size_t Polygon::point_count() const
{
    return points_.size();
}
}  // namespace USTC_CG
